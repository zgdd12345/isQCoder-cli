...
const MEMORY_SECTION_HEADER = '## isQCoder Added Memories';
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', GEMINI_DIR, 'ISQCODER.md');
        expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
        expect(result.fileName).toContain(
          path.join('mock', 'home', GEMINI_DIR),
        );
        expect(result.fileName).toContain('ISQCODER.md');
        expect(result.fileDiff).toContain('Index: ISQCODER.md');
        expect(result.fileDiff).toContain('+## isQCoder Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## isQCoder Added Memories');
