...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from '@isqcoder/isqcoder-cli-core/src/ide/detect-ide.js';

const CLI_IDE_COMPANION_IDENTIFIER = 'Google.@isqcoder/isqcoder-vscode';
  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the isQCoder Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('isQCoder IDE Companion');
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('isqcoder.runIsQCoderCLI', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run isQCoder.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run isQCoder in',
        });
      }
      ...
      if (selectedFolder) {
        ...
        const terminal = vscode.window.createTerminal({
          name: `isQCoder (${selectedFolder.name})`,
    ...
    vscode.commands.registerCommand('isqcoder.showNotices', async () => {
